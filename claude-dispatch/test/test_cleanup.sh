...
setup_dispatched_worktree() {
  ...
  git -C "$SANDBOX/code/org/r" worktree add -q -b dispatch/c \
    "$SANDBOX/code/org/r-wt-c" origin/main
}
...
test_cleanup_removes_a_clean_worktree() {
  ...
  [ ! -d "$SANDBOX/code/org/r-wt-c" ] || fail "worktree not removed"
}

test_cleanup_refuses_dirty_worktree() {
  setup_dispatched_worktree
  printf 'wip\n' >> "$SANDBOX/code/org/r-wt-c/README.md"
  local out
  out="$(DISPATCH_SEARCH_GLOB="$SANDBOX/code/*" \
  ...
  [ -d "$SANDBOX/code/org/r-wt-c" ] || fail "dirty worktree was removed anyway"
}
...
test_cleanup_force_removes_dirty_worktree() {
  ...
  [ ! -d "$SANDBOX/code/org/r-wt-c" ] || fail "worktree not removed with --force"
}

test_cleanup_refuses_unmerged_commits() {
  setup_dispatched_worktree
  printf 'work\n' >> "$SANDBOX/code/org/r-wt-c/README.md"
  git -C "$SANDBOX/code/org/r-wt-c" commit -qam "work"
  local out
  out="$(DISPATCH_SEARCH_GLOB="$SANDBOX/code/*" \
    "$ROOT/bin/dispatch-task.sh" --cleanup c --target r 2>&1)" \
